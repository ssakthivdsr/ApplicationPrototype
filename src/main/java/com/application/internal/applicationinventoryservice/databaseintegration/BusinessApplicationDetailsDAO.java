...
import com.application.internal.applicationinventoryservice.to.PGObject;
...
public class BusinessApplicationDetailsDAO {
	...
	public void storeBusinessApplicationData(BusinessApplicationDetailsTO businessApplicationDetailsTO)
		...
		for (int i = 0; i < channels.size(); i++) {
			...
			channelParams.put("volume", new Gson().fromJson(channels.get(i).getVolume().toString(), PGObject.class));
		...
		for (int i = 0; i < products.size(); i++) {
			...
			productParams.put("volume", new Gson().fromJson(products.get(i).getVolume().toString(), PGObject.class));
			productParams.put("writtenPremiumOfProducts", new Gson().fromJson(products.get(i).getwrittenPremiumOfProducts().toString(), PGObject.class));
		...
		for (int i = 0; i < transactions.size(); i++) {
			...
			transactionParams.put("volume", new Gson().fromJson(transactions.get(i).getVolume().toString(), PGObject.class));
		...
		for (int i = 0; i < users.size(); i++) {
			...
			userParams.put("volume", new Gson().fromJson(users.get(i).getVolume().toString(), PGObject.class));
